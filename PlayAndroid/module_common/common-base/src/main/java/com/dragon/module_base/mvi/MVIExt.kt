package com.dragon.module_base.mvi
...
fun <T, A> LiveData<T>.observeState(
    ...
    }.distinctUntilChanged().observe(lifecycleOwner::getLifecycle){ (a) ->
        action.invoke(a)
    }
}
...
fun <T, A, B> LiveData<T>.observeState(
    ...
    }.distinctUntilChanged().observe(lifecycleOwner::getLifecycle) { (a, b) ->
        action.invoke(a, b)
    }
}
...
fun <T, A, B, C> LiveData<T>.observeState(
    ...
    }.distinctUntilChanged().observe(lifecycleOwner::getLifecycle) { (a, b, c) ->
        action.invoke(a, b, c)
    }
}
...
fun <T> LiveData<List<T>>.observeEvent(lifecycleOwner: LifecycleOwner, action: (T) -> Unit) {
    this.observe(lifecycleOwner::getLifecycle) {
        it.forEach { event ->
            action.invoke(event)
        }
    }
